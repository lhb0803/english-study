...
- **@google-cloud/text-to-speech** — Neural2 voices for article audio (build-time only)
- **@vercel/blob** — hosts generated mp3s; URLs are saved in `latest.json`
...
GitHub Actions cron (Sun 21:00 UTC = Mon 06:00 KST)
  └─ npm run picks:build
       ...
       ├─ scripts/synthesize-audio.ts  (Google Neural2 → mp3 → Vercel Blob)
       └─ scripts/prune-audio.ts       (deletes blobs older than 12 weeks)
  └─ stefanzweifel/git-auto-commit-action
       commits public/picks/latest.json, public/picks/<date>.json,
       data/article-cache.json, data/audio-cache.json
...
7. **Client components need SSR guards.** Any component that reads zustand state must gate on a `mounted` flag or a `useEffect` to avoid hydration mismatches (see `StreakBanner`, `SaveButton`, `ListenButton`).
   ...
   - `ListenButton` (`src/components/ListenButton.tsx`) prefers `article.audioUrl` (Vercel Blob mp3 generated by Google Neural2 TTS at build time) and falls back to the browser `SpeechSynthesis` API when the url is missing (pruned audio or feature-flag off). On unmount it pauses the `<audio>` element and calls `speechSynthesis.cancel()` so playback stops on navigation.
...
9. **Workflow secrets.** `ANTHROPIC_API_KEY`, `GCP_TTS_KEY` (full service-account JSON pasted as a single string), and `BLOB_READ_WRITE_TOKEN` live in GitHub repo Secrets (Actions scope). Vercel does not need them — all Claude/TTS/Blob work happens in CI, not at runtime. Audio files live in Vercel Blob; only the public URL is committed inside `latest.json`.
10. **Audio cache + prune.** `scripts/synthesize-audio.ts` dedups by `{articleId, contentHash}` in `data/audio-cache.json` so rebuilds don't re-synthesize. `scripts/prune-audio.ts` runs after each successful build and deletes blobs (and cache entries) older than 12 weeks. If you change the retention window, update `RETENTION_WEEKS` in `prune-audio.ts` and the Vercel Blob storage estimate in README/CLAUDE.
...
| `scripts/synthesize-audio.ts` | Google TTS → Vercel Blob; cache in `data/audio-cache.json` |
| `scripts/prune-audio.ts` | Deletes blobs older than 12 weeks |
